import type { ParseAbiItem, AbiEvent, AbiEventParameterToPrimitiveType, AbiParameter } from 'viem';
...
type FieldTransform<TValue, TItem> = (args: { value: TValue; item: TItem }) => any;

type DeepTransformMap<T extends readonly AbiParameter[], TContext> = {
  [P in T[number] as P extends { name: string } ? P['name'] : never]: P extends {
    type: `${string}[]`;
...
export type EventTransformSchema<TEvent extends AbiEvent> = {
  [P in TEvent['inputs'][number] as P['name'] extends string ? P['name'] : never]: P extends {
    type: `${string}[]`;
    components: readonly AbiParameter[];
  }
    ...
    : P extends { type: 'tuple'; components: readonly AbiParameter[] }
      ? DeepTransformMap<P['components'], EventTransformSchema<TEvent>>
      : FieldTransform<AbiEventParameterToPrimitiveType<P>, EventTransformSchema<TEvent>>;
};
